...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Acent address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Acent account at a particular block.
    type Account {
    ...
    type Block {
        ...
        # Account fetches an Acent account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Acent account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Acent block by number or by hash. If neither is
