...
import (
	...
	"github.com/acent/go-acent/crypto/signify"
	"github.com/acent/go-acent/internal/build"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Acent contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Acent bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Acent Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "geth",
			Description: "Acent CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Acent private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Acent account management tool.",
		},
	}
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("go-acent requires at least Go version 1.13 and cannot")
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "acent/acent")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-acent sources.
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "acent"
...
type debMetadata struct {
	...
	// go-acent version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Acent Builds <fjl@acent.org>"
	}
	return debMetadata{
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.acent", "-v", "github.com/acent/go-acent/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/acent/go-acent/mobile")
