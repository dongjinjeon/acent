...
import (
	...
	"github.com/acent/go-acent/eth/fetcher"
...
// support all the operations needed by the Acent chain protocols.
...
// newHandler returns a handler for all Acent chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Acent handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Acent peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Acent peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if peer == nil {
		logger.Error("Acent peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Acent peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Acent peer removal failed", "err", err)
	}
...
func (h *handler) Stop() {
	...
	log.Info("Acent protocol stopped")
}
