...
// Package eth implements the Acent protocol.
package eth
...
import (
	...
	"github.com/acent/go-acent/eth/filters"
	"github.com/acent/go-acent/eth/gasprice"
	"github.com/acent/go-acent/eth/protocols/eth"
	"github.com/acent/go-acent/eth/protocols/snap"
...
// Acent implements the Acent full node service.
type Acent struct {
...
// initialisation of the common Acent object)
func New(stack *node.Node, config *ethconfig.Config) (*Acent, error) {
	// Ensure configuration values are compatible and sane
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run eth.Acent in light sync mode, use les.LightAcent")
	}
	...
	log.Info("Initialising Acent protocol", "network", config.NetworkId, "dbversion", dbVer)
...
// APIs return the collection of RPC services the acent package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *Acent) APIs() []rpc.API {
...
func (s *Acent) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *Acent) Etherbase() (eb common.Address, err error) {
	s.lock.RLock()
...
func (s *Acent) isLocalBlock(block *types.Block) bool {
...
// is a local account.
func (s *Acent) shouldPreserve(block *types.Block) bool {
...
func (s *Acent) SetEtherbase(etherbase common.Address) {
	s.lock.Lock()
	s.etherbase = etherbase
	s.lock.Unlock()
...
func (s *Acent) StartMining(threads int) error {
...
func (s *Acent) IsMining() bool      { return s.miner.Mining() }
func (s *Acent) Miner() *miner.Miner { return s.miner }
...
func (s *Acent) IsListening() bool                  { return true } // Always listening
func (s *Acent) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *Acent) Synced() bool                       { return atomic.LoadUint32(&s.handler.acceptTxs) == 1 }
func (s *Acent) ArchiveMode() bool                  { return s.config.NoPruning }
func (s *Acent) BloomIndexer() *core.ChainIndexer   { return s.bloomIndexer }

// Protocols returns all the currently configured
// network protocols to start.
func (s *Acent) Protocols() []p2p.Protocol {
