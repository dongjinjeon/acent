...
// Verify that Client implements the acent interfaces.
var (
	_ = acent.ChainReader(&Client{})
	_ = acent.TransactionReader(&Client{})
	_ = acent.ChainStateReader(&Client{})
	_ = acent.ChainSyncReader(&Client{})
	_ = acent.ContractCaller(&Client{})
	_ = acent.GasEstimator(&Client{})
	_ = acent.GasPricer(&Client{})
	_ = acent.LogFilterer(&Client{})
	_ = acent.PendingStateReader(&Client{})
	// _ = acent.PendingStateEventer(&Client{})
	_ = acent.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Acent Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new acent service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == acent.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != acent.NotFound {
		t.Fatal("error should be acent.NotFound")
	}
}
