...
import (
	...
	vfs "github.com/acent/go-acent/les/vflux/server"
	...
	"github.com/acent/go-acent/p2p/nodestate"
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Acent peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light Acent handshake failed", "err", err)
		return err
	}
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light Acent peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light acent response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Acent message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Acent message arrived", "code", msg.Code, "bytes", msg.Size)
