...
// Package les implements the Light Acent Subprotocol.
package les
...
import (
	...
	"github.com/acent/go-acent/les/vflux"
...
func New(stack *node.Node, config *ethconfig.Config) (*LightAcent, error) {
...
func (s *LightAcent) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightAcent) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightAcent) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the acent package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightAcent) APIs() []rpc.API {
...
func (s *LightAcent) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightAcent) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightAcent) TxPool() *light.TxPool              { return s.txPool }
func (s *LightAcent) Engine() consensus.Engine           { return s.engine }
func (s *LightAcent) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightAcent) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightAcent) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols returns all the currently configured network protocols to start.
func (s *LightAcent) Protocols() []p2p.Protocol {
...
// light acent protocol implementation.
func (s *LightAcent) Start() error {
