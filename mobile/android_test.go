...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.acent", new NodeConfig());
			node.start();
			...
			AcentClient ec = node.getAcentClient();
	...
	// signatures too. Regression test for go-acent issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/acent/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.acent", "github.com/acent/go-acent/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "acent", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
