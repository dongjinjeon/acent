...
// geth is the official command-line client for Acent.
package main
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-acent command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2021 The go-acent Authors"
...
func prepare(ctx *cli.Context) {
	...
	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Geth on Acent mainnet...")
	}
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Acent node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("Acent service not running: %v", err)
		}
