...
// nodeDockerfile is the Dockerfile required to run an Acent node.
var nodeDockerfile = `
FROM acent/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.acent/keystore/ && cp /signer.json /root/.acent/keystore/' >> geth.sh && \{{end}}
...
// an Acent node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.acent{{if .Ethashdir}}
...
// deployNode deploys a new Acent node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.acent"],
