...
import (
	...
	"github.com/acent/go-acent/accounts/usbwallet/trezor"
...
// or whether the Acent app was not started on it.
...
// Acent address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AcentAddress)
	if _, err := w.trezorExchange(&trezor.AcentGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.AcentTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AcentTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
