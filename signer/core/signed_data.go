...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useAcentV = true // Default to use V = 27 or 28, the legacy Acent format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useAcentV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the Acent PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useAcentV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Acent ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Acent Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useAcentV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
