...
// PublicAcentAPI provides an API to access Acent related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicAcentAPI struct {
	b Backend
}

// NewPublicAcentAPI creates a new Acent protocol API.
func NewPublicAcentAPI(b Backend) *PublicAcentAPI {
	return &PublicAcentAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicAcentAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicAcentAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Acent ECDSA signature for:
// keccack256("\x19Acent Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/acent/go-acent/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Acent Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/acent/go-acent/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Acent signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Acent blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Acent blockchain API.
...
//
// https://github.com/acent/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Acent APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Acent APIs exposed over the private
// debugging endpoint.
...
// Version returns the current acent protocol version.
